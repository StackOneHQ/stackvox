...
The optional `mlx` backend runs any text-to-speech model that
[MLX Audio](https://github.com/Blaizzy/mlx-audio) can load from a local
directory, including ones that support voice cloning and style instructions.
Kokoro (ONNX) remains the default backend and dependency set.

```bash
pip install 'stackvox[mlx]'
...
stackvox speak \
  ...
  --out cloned.wav \
  "Hello from a local MLX model."
```

> **Model licenses:** stackvox never downloads, bundles, or recommends MLX
> models. You supply the model directory, and its license (which may restrict
> commercial use) applies to your use of it. Some models also need their own
> extra Python packages; MLX Audio names them if they are missing.

Keep a large model resident for repeated requests by selecting it when the
daemon starts. The backend and model cannot be changed without restarting the
daemon. Generation options given to `serve` become the daemon's defaults, and
any `say` request can override them:

```bash
stackvox serve --backend mlx --model-dir /path/to/model

stackvox say \
  --reference-audio /path/to/reference.wav \
  --reference-text "The exact transcript of the reference audio." \
  "This request uses the resident model."
```

Generation options (`--reference-audio`, `--reference-text`, `--instruct`,
`--temperature`, `--top-p`, `--top-k`) are forwarded only when set, so each
...
Some models need custom loading, such as renamed weights, a codec path, or a
tokenizer quirk. Rather than carrying model-specific code, stackvox accepts an
adapter script that you write and keep locally, for example under
`~/.config/stackvox/adapters/`:

```python
# my_adapter.py
from pathlib import Path
from stackvox.mlx_backend import swapped


def load(model_dir: Path, load_model):
    # Patch whatever the model needs for the duration of the load, then
    # delegate to MLX Audio's loader (or replace it entirely).
    from some_library import SomeClass

    with swapped(SomeClass, "some_method", lambda *args, **kwargs: ...):
        return load_model(model_dir)
```

```bash
stackvox speak --backend mlx --model-dir /path/to/model \
  --model-adapter ~/.config/stackvox/adapters/my_adapter.py "Hello"
```

`load(model_dir, load_model)` must return the loaded model. `swapped` patches
an attribute and restores it afterwards, even on error. Adapters run as normal
Python with your privileges and are only used when passed explicitly; nothing
is picked up from a model directory.

The Python API uses the same waveform contract as Kokoro:

```python
from pathlib import Path
from stackvox import Stackvox

tts = Stackvox(backend="mlx", model_dir=Path("/path/to/model"))
samples, sample_rate = tts.synthesize(
    "A calm and natural delivery.",
...
**Optional MLX backend.** `stackvox[mlx]` runs on [`mlx-audio`](https://github.com/Blaizzy/mlx-audio) (MIT, © Prince Canuma), [`mlx`](https://github.com/ml-explore/mlx) and [`mlx-lm`](https://github.com/ml-explore/mlx-lm) (MIT, © Apple), and [`transformers`](https://github.com/huggingface/transformers) (Apache 2.0, © Hugging Face). stackvox does not download, bundle, or redistribute any MLX model; the license of whichever model you supply governs your use of it.
